...
def draw_neural_net(ax, left, right, bottom, top, layer_sizes, coefs_=None,
                    intercepts_=None, n_iter_=None, loss_=None):
    '''
    ...
    for n, layer_size in enumerate(layer_sizes):
        ...
        for m in range(layer_size):
            ...
            if n == 0:
                plt.text(left-0.125, layer_top - m*v_spacing, r'$x_{{{}}}$'.format(m+1), fontsize=15)
            elif (n_layers == 3) & (n == 1):
                plt.text(n*h_spacing + left+0.00, layer_top - m*v_spacing+ (v_spacing/8.+0.01*v_spacing), r'$h_{{{}}}^{{({})}}$'.format(m+1, n), fontsize=15)
            elif n == n_layers -1:
                plt.text(n*h_spacing + left+0.10, layer_top - m*v_spacing, r'$y_{{{}}}$'.format(m+1), fontsize=15)
    ...
    for n, layer_size in enumerate(layer_sizes):
        if n < n_layers - 1:
            ...
            plt.text(x_bias-(v_spacing/8.+0.10*v_spacing+0.01), y_bias, r'$b^{{({})}}$'.format(n+1), fontsize=15)
            ax.add_artist(circle)   
    # Edges
    # Edges between nodes
